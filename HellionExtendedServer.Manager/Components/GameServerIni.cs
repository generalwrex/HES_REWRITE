    ...
    public class GameServerIni
    {
        ...
        */
        ...
        private object this[string property]
        {
            get
            {
                var myPropInfo = typeof(GameServerIni).GetProperty(property);
        ...
        public bool Save(bool ignoreFileExists = false, bool backupIni = true)
        {
            try
            {
                ...
                {
                    Log.Error("[ERROR] Hellion Extended Server[GameServerIni]: GAMESERVER.INI Does not exist!");
                    return false;                
                }
            }
            catch (IOException ex)
            {
                Log.Error(ex, "Could not save GameServer.Ini as the file is in use.");
            }
            catch (Exception ex)
            {
                Log.Error(ex, "[ERROR] Hellion Extended Server[GameServerIni]: Could not save config settings.");                
            }
        ...
        public void Load()
        {
            try
            {
                ...
                {
                    Log.Warn("GameServer.Ini wasn't found! Creating a new one based on Defaults. ");
                    LoadDefaults();
                    Save(true, false);
                }
            }
